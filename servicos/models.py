...
from usuarios.models import Refugiado, Voluntario # Importa perfis

# --- MODELO: HABILIDADE (Tabela de apoio) ---
class Habilidade(models.Model):
    """ Define o tipo de ajuda oferecida. (Integridade de Entidade/Domínio) """
    nome = models.CharField(max_length=100, unique=True)
    
    def __str__(self):
        return self.nome

# --- MODELO: SERVICO (Catálogo - Relação 1:N com Habilidade) ---
class Servico(models.Model):
    """ Itens no catálogo que o refugiado pode solicitar. """
    nome = models.CharField(max_length=100)
    descricao = models.TextField()
    # Integridade Referencial: on_delete=SET_NULL (a FK é zerada se a Habilidade for excluída)
    habilidade_necessaria = models.ForeignKey(Habilidade, on_delete=models.SET_NULL, null=True, blank=True)
    
    def __str__(self):
        return self.nome

# --- MODELO: SOLICITACAO_SERVICO (Tabela de Ligação N:1) ---
class SolicitacaoServico(models.Model):
    ...
    def __str__(self):
        return f"Solicitação {self.id} de {self.refugiado.usuario.first_name}"

# --- MODELO: VOLUNTARIO_HABILIDADE (Tabela de Ligação N:M) ---
class VoluntarioHabilidade(models.Model):
    ...
    class Meta:
        # Integridade de Entidade: O par Voluntário-Habilidade deve ser único
